...
function calculate_populations(sol, engine_basis::NLevelBasis, load_basis::FockBasis)
    ...
    # Precompute projectors for engine and load
    engine_projectors = [
        projector(nlevelstate(engine_basis, n)) ⊗ one(load_basis) 
        for n in 1:engine_basis.N
    ]
    load_projectors = [
        one(engine_basis) ⊗ projector(fockstate(load_basis, n )) 
        for n in load_basis.offset:load_basis.N
    ]

    # Initialize 2D matrices for populations
    engine_populations = zeros(Float64, engine_basis.N, length(sol))
    load_populations = zeros(Float64, load_basis.N + 1 - load_basis.offset, length(sol))

    # Iterate over sol with index
    for (idx, ρ) in enumerate(sol)
        # Compute engine populations
        engine_populations[:, idx] .= real.(expect.(engine_projectors, Ref(ρ)))
        
        # Compute load populations
        load_populations[:, idx] .= real.(expect.(load_projectors, Ref(ρ)))
    end
...
function ergotropy(ρ::Operator, H::Operator)
    # the energy of the system in its current state
    Ecurrent = tr(ρ * H)

    # the density matrix with the eigenvalues sorted in decreasing order
    λ_sorted = sort(eigvals(Matrix(ρ.data)), by=abs, rev=true)
    ε_sorted = sort(eigvals(Matrix(H.data)), by=abs)
...
function linreg(tspan::Vector{Float64}, arr::Vector{Float64})
    ...
    if length(tspan) != length(arr)
        throw(DimensionMismatch("tspan and arr must have the same length. Found $(length(tspan)) and $(length(arr))"))
    end
...
# Function to animate the load dynamics as a gif
function animate_load_populations_line(t, load_populations, filename="population_dynamics.gif", fps=30)
    # Extract the size of the data
    num_states, num_time_slices = size(load_populations)

    # Create the animation
    anim = @animate for i in 1:num_time_slices
        plot(
            0:num_states-1, load_populations[:, i],  # Plot the distribution at time step i
            xlabel=L"n",
            ylabel=L"p(n)",
            title="Load population at time: $(round(t[i], digits=2))",  # Use the time slice index as the title
            legend=false,
            ylim=(0, 1),  # Fixed y-axis limits for probabilities
            xlim=(0, num_states-1),  # x-axis limits based on the number of states
            color=:blue,
            lw=2,
            marker=(:circle, 5),
            size=(600, 400),
